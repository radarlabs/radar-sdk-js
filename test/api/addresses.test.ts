  ...
  describe('validateAddress', () => {
    describe('params are not provided', () => {
      it('should have undefined params and return an autocomplete response', async () => {
        ...
        expect(Http.request).toHaveBeenCalledWith({ ...httpRequest, data: { countryCode: undefined, stateCode: undefined, city: undefined, number: '841', postalCode: undefined, street: 'Broadway', unit: undefined, addressLabel: undefined } });
    ...
    describe('params are provided', () => {
      it('number, street, unit provided should return an autocomplete response', async () => {
        ...
        expect(Http.request).toHaveBeenCalledWith({ ...httpRequest, data: { countryCode: 'US', stateCode: 'NY', city: 'New York', postalCode: '10010', number: '841', street: 'Broadway', unit: '7', addressLabel: undefined } });
        expect(response).toEqual(validateResponse);
      }, 10000);
    });
    ...
    it('addressLabel provided should return an autocomplete response', async () => {
      ...
      expect(Http.request).toHaveBeenCalledWith({ ...httpRequest, data: { countryCode: 'US', stateCode: 'NY', city: 'New York', postalCode: '10010', addressLabel: '841 Broadway Unit 7', number: undefined, street: undefined, unit: undefined } });
      expect(response).toEqual(validateResponse);
    });
  });
});
